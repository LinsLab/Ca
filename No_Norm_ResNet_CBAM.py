...
class ca2_resnet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = Conv_block(in_channels=2, out_channels=8, stride=1)
...
pd_test = pd.read_csv(r"No_Norm_realdata_testdata.csv").to_numpy()
df_test_x = pd_test[:, 100:300].reshape(-1, 2, 100)
...
net.load_state_dict(torch.load("No_Norm_ResNet_CBAM.pth"), strict=False)
